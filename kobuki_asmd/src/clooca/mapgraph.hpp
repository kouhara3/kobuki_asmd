...
public:
	...
	void dijkstra( int s ){
		...
		while(1){
			...
			for( int j=0; j<4; j++ ){
				...
				if( (n != NULL) && (n->block->getMark() != OBSTACLE) ){
	...
	Block* getNearestUnknownBlock( Block* now ){
		...
		for( int i=0; i<node_size; i++ ){
			if( (graph[i].block->getMark() == UNKNOWN) && (min_cost < graph[i].cost) ){
				min_cost = graph[i].cost;
				nearest_unknown_node = i;
			}
		}

		if( nearest_unknown_node > 0 ) return( graph[nearest_unknown_node].block );
		else return( NULL );
	}
